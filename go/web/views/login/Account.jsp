                ...
                loginName: [
                    ...
                    <el-form-item label="类型" prop="type">
                        <el-select v-model="ruleForm.type"
                                   placeholder="请选择账号类型">
                            <el-option
                                    v-for="item in selectData.accountType"
                                    :key="item.id"
                                    :label="item.name"
                                    :value="item.id">
                            </el-option>
                    ...
                    <el-form :inline="true" :model="formInline" class="el-form--inline" label-width="100px" label-position="right"
                        ...
                        <el-col :span="12">
                            <el-form-item label="登录账号">
                                <el-input v-model="formInline.loginName" placeholder="登录账号"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
        ...
        methods: {
            ...
            select:function () {
                ...
                return {
                    ...
                    initSelect:function () {
                        // 加载  账号类型
                        t.$http.post('o/accountType/list').then(function (res) {
                            this.selectData.accountType = res.body.data;
                            var accountTypeIndex = this.selectData.accountTypeIndex = {};
                            this.selectData.accountType.forEach(
                                function (item) {
                                    accountTypeIndex[item.id] = item;
                                }
                            );
                            console.log(res);
                        })
            ...
            query: function () {
                ...
                // 开始查询
                this.$http.post('o/account/listByPage', params).then(function (res) {
            ...
            handleSizeChange: function (val) { // 每页条数 变更事件
                this.tableData.pageSize = val;
                console.log('每页' + val);
                this.query(); // 刷新
            }
            ,
            handleCurrentChange: function (val) { // 跳页事件
                console.log('当前页: ' + val);
            ...
            submitForm: function () {
                var t = this;
                t.dialogFormVisible = false; // 关闭窗口 避免重复提交
                this.$refs['ruleForm'].validate(function (valid) {
                    if (valid) {
                        var url = 'o/account/';
                        var params = JSON.parse(JSON.stringify(t.ruleForm));
                        var isUpdate;
                                ...
                                try {
                                    if (res.body.success) {
                                        if (isUpdate) {
                                            ...
                                            t.currentRow.typeBean = t.selectData.accountTypeIndex[params.type];
                                        }
                                        else
                                            t.query(); // 刷新表单
                                        t.msgs().open2("提交成功");
            ...
            del: function () {
                ...
                this.$http.post("o/account/del/" + this.currentRow.id)
                    .then(function (res) {
                        console.log(res);
                        try {
                            if (res.body.success) {
